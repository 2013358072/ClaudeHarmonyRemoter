...
- [x] 三套主题配色 + 四档字号，本地持久化
- [x] 底部导航（对话 / 我的）
- [x] **项目管理** —— 分组抽屉折叠、左滑置顶 / 重命名 / 移动分组、搜索
- [x] **文件夹筛选** —— 只显示指定目录下的会话
- [x] **Markdown 渲染** —— 模型回复的标题/代码块/列表/引用/行内标记
- [x] **列表虚拟化** —— LazyForEach，长列表不再一次性实例化全部条目
- [x] **扫码配对** —— Scan Kit 扫二维码一步直达
- [x] **消息通知** —— 对话级开关（默认关闭），切后台后授权请求与任务完成会提醒

尚未完成：

- [ ] 鸿蒙端真机联调（签名已就绪，待连机验证）
- [ ] 离线推送（当前是本地通知，App 被系统回收后收不到；离线推送需接入华为 Push Kit）
